...
for cut in range(num_cuts):
    ...
    flash_pe_threshold = 30.0 + (run * 2)
    # flash_pe_threshold - scan between 30.0 - 70.0 PE
    loop_cut.append(flash_pe_threshold)
...
f1_axes.set_title("Efficiency vs Flash PE")
f1_axes.set_autoscaley_on(False)
f1_axes.set_ylim([9, 11])
f1_axes.set_xlabel("Flash PE")
f1_axes.set_xlim([28, 72])
f1_axes.set_ylabel("Efficiency")
figure_1.savefig("plots/final_efficiency_flash_pe.pdf")

figure_2 = plt.figure()
plt.plot(loop_cut, purity_final, 'bo')
f2_axes = figure_2.add_subplot(111)
f2_axes.set_title("Purity vs Flash PE")
f2_axes.set_autoscaley_on(False)
f2_axes.set_ylim([54, 56])
f2_axes.set_xlabel("Flash PE")
f2_axes.set_xlim([28, 72])
f2_axes.set_ylabel("Purity")
figure_2.savefig("plots/final_purity_flash_pe.pdf")
